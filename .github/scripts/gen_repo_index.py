...
def main():
    ...
    parts = [
        ...
        # title bar
        f'<circle cx="{PAD+8}" cy="26" r="5" fill="#f85149"/>',
        f'<circle cx="{PAD+24}" cy="26" r="5" fill="#d29922"/>',
        f'<circle cx="{PAD+40}" cy="26" r="5" fill="#7ee787"/>',
        f'<text x="{PAD+62}" y="31" font-family="ui-monospace,Consolas,monospace" font-size="14" fill="#7ee787">siguatepeque@github:~$ ls -la --sort=activity</text>',
        f'<line x1="{PAD}" y1="44" x2="{W-PAD}" y2="44" stroke="#21262d"/>',
        f'<text x="{PAD}" y="60" font-family="ui-monospace,Consolas,monospace" font-size="11" fill="#8b949e">name</text>',
        f'<text x="{W-PAD}" y="60" text-anchor="end" font-family="ui-monospace,Consolas,monospace" font-size="11" fill="#8b949e">lang · ★ · pushed</text>',
    ]

    y = HEAD + PAD - 6
    if not repos:
        parts.append(
            f'<text x="{PAD}" y="{y}" font-family="ui-monospace,Consolas,monospace" font-size="13" fill="#f85149">index unavailable: {esc(err)[:80]}</text>'
        )
    else:
        for r in repos:
            name = r["name"]
            ...
            desc = trunc(r.get("description"), 78)
            ...
            parts += [
                ...
                f'  <text x="{PAD}" y="{y+2}" font-family="ui-monospace,Consolas,monospace" font-size="14" fill="#58a6ff">{esc(name)}</text>',
                f'  <text x="{PAD}" y="{y+19}" font-family="ui-monospace,Consolas,monospace" font-size="11" fill="#8b949e">{esc(desc)}</text>',
                f'</a>',
                f'<circle cx="{W-PAD-150}" cy="{y-2}" r="5" fill="{lc}"/>',
                f'<text x="{W-PAD-138}" y="{y+2}" font-family="ui-monospace,Consolas,monospace" font-size="12" fill="#c9d1d9">{esc(lang)}</text>',
                f'<text x="{W-PAD}" y="{y+2}" text-anchor="end" font-family="ui-monospace,Consolas,monospace" font-size="12" fill="#d29922">★{stars} <tspan fill="#8b949e">{pushed}</tspan></text>',
            ]
            y += ROW_H

    parts.append(
        f'<text x="{W/2}" y="{H-10}" text-anchor="middle" font-family="ui-monospace,Consolas,monospace" font-size="10" fill="#484f58">rebuilt daily by github actions · no projects hardcoded</text>'
    )
    parts.append("</svg>")
